...
import { AiOutlineUser, AiOutlineSwap, AiOutlineLogout, AiOutlineAppstore, AiOutlineSolution } from "react-icons/ai";
    ...
    <div className="flex fixed top-0 left-0 h-screen w-16 flex-col justify-between border-e border-gray-50 bg-white">
      <div>
        <div className="inline-flex size-16 items-center justify-center">
          <span className="grid size-10 place-content-center rounded-lg bg-gray-100 text-xs text-gray-600">{initial}</span>
        </div>
          ...
          <div className="px-2">
            <div className="py-4">
              {navItems.slice(0, 1).map(({ href, icon, label }) => {
                ...
                return (
                  <Link key={href} href={href} className={`group relative flex justify-center rounded-sm px-2 py-1.5 ${isActive ? "bg-blue-50 text-blue-700" : "text-gray-500 hover:bg-gray-50 hover:text-gray-700"}`}>
                    {icon}
                    <span className="invisible absolute start-full top-1/2 ms-4 -translate-y-1/2 rounded-sm bg-gray-900 px-2 py-1.5 text-xs font-medium text-white group-hover:visible">{label}</span>
                  </Link>
                );
              })}
            </div>
            ...
            <ul className="space-y-1 border-t border-gray-100 pt-4">
              {navItems.slice(1).map(({ href, icon, label }) => {
                ...
                return (
                  <li key={href}>
                    <Link href={href} className={`group relative flex justify-center rounded-sm px-2 py-1.5 ${isActive ? "bg-blue-50 text-blue-700" : "text-gray-500 hover:bg-gray-50 hover:text-gray-700"}`}>
                      {icon}
                      <span className="invisible absolute start-full top-1/2 ms-4 -translate-y-1/2 rounded-sm bg-gray-900 px-2 py-1.5 text-xs font-medium text-white group-hover:visible">{label}</span>
                    </Link>
                  </li>
