...
class TransactionService {
  async getAllByUser(userId, page = 1, limit = 10, search = "") {
    ...
    if (search) {
      whereClause[Op.or] = [{ note: { [Op.like]: `%${search}%` } }, { desc: { [Op.like]: `%${search}%` } }];
    }
  ...
  async create(data) {
    ...
    if (data.type === "expense" && totalIncome < totalExpense + amountToAdd) {
      throw new BadRequest("Income bulan ini tidak mencukupi");
    }
  ...
  async getMonthlySummary(userId) {
    ...
    const saving = Math.floor(Math.max(0, totalIncome - totalExpense) * 0.3 + totalIncome * 0.05);
